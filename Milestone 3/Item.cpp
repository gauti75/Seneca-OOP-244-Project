	...
	std::ostream& Item::write(std::ostream& ostr)const {
		...
		if (err) {
			...
			if (diaplayType == POS_LIST) {
				...
				if (tax == true) {
					ostr << " X |  " << right << quantity() << "|" << setw(9) << fixed << setprecision(2) << cost() * quantity() << "|";
				}
				else if (tax == false) {
				...
				ostr << "=============v" << endl;
				ostr << "Name: " << setw(13) << name << endl;
				ostr << "Sku: " << setw(12) << SKU << endl;
				ostr << "Price: " << setw(11) << price << endl;
				if (tax) {
					ostr << "Price + tax: " << cost() << endl;
				}
				else {
					ostr << "Price + tax: " << "N/A" << endl;
				}
				ostr << "Stock Qty:" << setw(5) << Quantity << endl;
