...
namespace sdds {
	...
	std::ostream& Date::display(std::ostream& ostr = cout)const {
		if (!error()) {
			if (date_only == true) {
				if (month < 10&&day>9) {
					ostr << year << "/" <<"0" << month << "/" << setw(2) << setfill('0') << day <<setfill(' ');
				}
				else if (month < 10 && day < 9) {
					ostr << year << "/" << "0" << month << "/" << "0" << day << setfill(' ');
				}

				else {
					ostr << year << "/" << setw(2) << setfill(' ') << month << "/" << setw(2) << setfill(' ') << day;
				}
				
			}
			else if (date_only == false) {
				if (month < 10) {
					ostr << year << "/" << "0" << month << "/" << setw(2) << setfill(' ') << day << ", " << setw(2) << setfill(' ') << hours << ":" << setw(2) << setfill(' ') << minutes;
				}
				else {
					ostr << year << "/" << setw(2) << setfill(' ') << month << "/" << setw(2) << setfill(' ') << day << ", " << setw(2) << setfill(' ') << hours << ":" << setw(2) << setfill(' ') << minutes;
				}
			...
			if (date_only == true) {
				ostr << error() << "(" << year << "/" << setw(2) << setfill(' ') << month << "/" << setw(2) << setfill(' ') << day << ")";
			}
			else if (date_only == false) {
				ostr << error() << "(" << year << "/" << setw(2) << setfill(' ') << month << "/" << setw(2) << setfill(' ') << day << ", " << setw(2) << setfill(' ') << hours << ":" << setw(2) << setfill(' ') << minutes << ")";
			}
		}
	...
	Date& Date::operator=(Date& rhs_date) {

		if (this != &rhs_date) {
			this->set_day(rhs_date.day);
			this->set_hours(rhs_date.hours);
			this->set_month(rhs_date.month);
			this->set_year(rhs_date.year);
			date_only = true;
