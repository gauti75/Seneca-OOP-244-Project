...
namespace sdds {
	...
	const char* Item::get_sku() {
		return SKU;
	}

	char* Item::return_name()const {
		return name;
	}

	double Item::cost()const {
		return price * (1 + tax * TAX);
	}
	...
	*/
	...
	void Item::operator+=(int added_quantity) {
		...
		if (err) {
			...
			if (diaplayType == POS_LIST) {
				...
				if (tax == true) {
					...
					ostr << "   |" <<setw(4) << right << quantity() << "|" << setw(9) << fixed << setprecision(2) << cost() * quantity() << "|";
				}
				...
				if (tax) {
					ostr << left << setw(13)<< "Price + tax: "<<setprecision(2)<<fixed << cost() << endl;
				}
				else {
	...
	istream& Item::read(istream& is) {
		...
		do {
			cout << "> ";
			is.getline(temp_sku, MAX_SKU_LEN);
